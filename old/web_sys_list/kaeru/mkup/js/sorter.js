...
var table=function(){
	...
	sorter.prototype.init=function(){
		...
		this.tbody = this.table.getElementsByTagName('tbody')[0]; /* theadを取得 */
	...
	sorter.prototype.work=function(col){
		...
		for(i = 0; i < this.rownum; i++){
			this.a[i].index=i; 
			this.a[i].value = getCelContent(this.brows[i].cells[col]);
		}
		...
		if(this.seq[col]=='desc'){
			this.seq[col]='asc';
			this.a.reverse(compare); //ソート実行
			span.innerHTML = this.deschtml;
		}else{
			this.seq[col]='desc';
			this.a.sort(compare); //ソート実行
		...
		for(i=0; i < this.rownum; i++){
			var row=this.brows[this.a[i].index].cloneNode(true);
	...
	sorter.prototype.mover=function(row){
		...
		for(var i = 0; i < this.colnum; i++){
			...
			var rgb = getBgColor(cell);
			rgb.r = CalculateColor(rgb.r);
			rgb.g = CalculateColor(rgb.g);
			rgb.b = CalculateColor(rgb.b);
	...
	sorter.prototype.hmover=function(col){
		var rgb = getBgColor(this.head[col]);
		rgb.r = CalculateColor2(rgb.r);
		rgb.g = CalculateColor2(rgb.g);
		rgb.b = CalculateColor2(rgb.b);
	...
	function v2d(v)
	{
		...
		ymd = 0;
		if (m[3]&&!isNaN(m[3])) ymd = wa2sei(m[2],parseInt(m[3]));
		if (m[4]&&!isNaN(m[4])) ymd = ymd*100+parseInt(m[4]);
		if (m[5]&&!isNaN(m[5])) ymd = ymd*100+parseInt(m[5]);
		return ymd;
	}
	/* 和暦を西暦に変換 */
	function wa2sei(wa,y)
	{
		var gen=[];
		var nen=[];
