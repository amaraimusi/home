 ...
 * - embedded チャット画面を埋込化する
 * - smallWindow チャット画面を小窓化する
...
var ShchX = function(){
	...
	this.prop={
			...
			'host':'',//ホスト
			'src_form_flg':0,//画面形態フラグ  0:小窓形態 1:埋込形態 2:スマホ形態
	 ...
	 * SHCHゲストチャットを起動する
	 * 
	 * @param prm_prop プロパティ
	 * ◇propの中身
	 * - info_room_id 案内ルームID
	 * - hash ハッシュキー
	 * - host ホスト（例： 「/shch/」、「http://192.168.11.199/shch/」
	 * - src_form_flg 画面形態フラグ  0:小窓形態 1:埋込形態 2:スマホ形態
	 * - debug_mode デバッグモード： 0:OFF 1:リロード数などを表示   2:システムエラーのダンプを表示
	 * - debug_slc デバッグ出力要素名
	 */
	this.run=function(prm_prop){
		...
		//画面形態の変更
		this._changeScreenForm(this.prop.src_form_flg);
		
		//shchコンパクト区分のクリックイベント
		$('#shch_guest_chat').click(function(e){
			
			shchX.aiInfo.base_ut=0;//自動インターバル制御のリセット
	...
	//プロパティへローカルストレージや引数の値をセットする。
	this._setProp=function(param_prop){
		
		//ローカルストレージに保存してあるプロパティが存在するなら、メンバプロパティにマージする。
		var info_room_id = param_prop['info_room_id'];
		var key="shch_guest_prop_" + info_room_id;
		var json=localStorage.getItem(key);
		if(json!=null){
			var lsProp=$.parseJSON( json );
			$.extend(this.prop, lsProp);//メンバプロパティへマージする。
		}
		
		//メンバプロパティへ引数プロパティをマージする
		$.extend(this.prop, param_prop);

		//キーはセットや削除で使うのでメンバで保持する。
		this.propKey=key;
		
		//初回のチャットデータ表示のため、最終日時をクリア。
	...
	/**
	 * 画面形態の変更 
	 * param src_form_flg 画面形態フラグ  0:小窓形態 1:埋込形態 2:スマホ形態
	 */
	this._changeScreenForm=function(src_form_flg){
		...
		if(src_form_flg==1){
			...
			//小窓ボタンを表示し、埋込アイコンを非表示
			$('#shch_small_window_btn').show();
			$('#shch_embedded_btn').hide();
		...
		//2:スマホ形態
		else if(src_form_flg==2){
			
			//オープンモードにする
			this._onOpenMode();
			
			//小窓ボタンと埋込アイコンを非表示
			$('#shch_small_window_btn').hide();
			$('#shch_embedded_btn').hide();
		...
		//0:小窓形態
			...
			//埋込ボタンを表示し、小窓アイコンを非表示
			$('#shch_embedded_btn').show();
			$('#shch_small_window_btn').hide();
	...
	//ドラッグ機能の無効化
	this._offDraggable=function(){
		
		var draggableDiv = $('#shch_guest_chat').draggable();
		draggableDiv.draggable('destroy');//ドラッグ機能無効化
	...
	this.embedded = function(){
		...
		//画面形態フラグ  0:小窓形態 1:埋込形態 2:スマホ形態
		this.prop.src_form_flg=1;
	 ...
	 * チャット画面を小窓化する
	 */
	this.smallWindow = function(){
		
		//画面形態フラグ  0:小窓形態 1:埋込形態 2:スマホ形態
		this.prop.src_form_flg=0;
	...
	/**
	 * 現在のモードに合わせて画面サイズを変更する
	 * @param mode モード（省略可） 0:コンパクトモード,1:オープンモード,2：設定モード
	 */
	this._setScreenSize=function(mode){
		...
		if(mode==0){
			$('#shch_guest_chat').css({
				'width':this.prop.compact_width,
				'height':this.prop.compact_height,
			});
		...
		//設定モード
			...
			if(this.prop.src_form_flg==0){
				...
				$('#shch_guest_chat').css({
					'width':this.prop.config_mode_width,
					'height':this.prop.config_mode_height,
				});
			}
...
function shchWriteHtml(){
	var htmlStr=
		...
		"					<button id=\"shch_embedded_btn\" type=\"button\" class=\"btn btn-primary btn-xs pull-right\" onclick='shchEmbedded()'>" +
		"						<span class=\"glyphicon glyphicon-fullscreen\" title='埋め込み化'></span></button>" +
		"					<button id=\"shch_small_window_btn\" type=\"button\" class=\"btn btn-primary btn-xs pull-right\" onclick='shchSmallWindow()'>" +
		"						<span class=\"glyphicon glyphicon-resize-small\" title='小窓化'></span></button>" +
		"				</div>" +
		"			</div>" +
 ...
 * 
 */
function shchGuestChat(prop){

	shchWriteHtml();//チャット画面のHTMLを書きだす。
